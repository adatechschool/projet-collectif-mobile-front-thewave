...
fun main() {
    val spotJSONObject = JSONObject(spotsArray)

    val spotObject = spotJSONObject.getJSONArray("Spot")

    for (i in 0 until(spotObject!!.length())) {
        val theSpot = Spot();
        val id = spotObject.getJSONObject(i).getString("id");
        theSpot.id = id;

        val fields = spotObject.getJSONObject(i).getJSONObject("fields");
        theSpot.fields = fields;

        val createdTime = spotObject.getJSONObject(i).getString("createdTime");
        theSpot.createdTime = createdTime;
